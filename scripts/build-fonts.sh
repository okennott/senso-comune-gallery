...
#
# The ITALIC, added 20 September 2026. The site asks for italic in four places
# — the hero's Dutch original, the Tribute epigraph, and every work title, on
# the index and as an h1 — and shipped no italic face, so the browser slanted
# the roman in all four. A faux italic of a high-contrast serif is a poor
# imitation of a drawing that already exists. Its axes are narrowed harder than
# the roman's because its uses are known exactly:
#   - opsz 14-30 covers every size an italic is actually set at (16.8 and 28.8);
#   - wght 400 only. No italic on this site is ever bold, and keeping 400-700
#     costs 57.0 KB instead of 31.4 KB — 82% more for nothing.
# It is NOT preloaded: it is wanted below the fold on most pages and swaps in.
#
# Measured output: fraunces 50.7 KB + inter 29.1 KB = 79.8 KB render-blocking,
# plus fraunces-italic 31.4 KB lazily.
#
...
curl -sL -o "$TMP/Fraunces-Italic.ttf" \
...
fonttools varLib.instancer -q -o "$TMP/fri.ttf" "$TMP/Fraunces-Italic.ttf" \
  SOFT=0 WONK=1 opsz=14:30 wght=400
...
pyftsubset "$TMP/fri.ttf" --output-file="$OUT/fraunces-italic-latin.woff2" \
  --flavor=woff2 --with-zopfli --unicodes="$LATIN" \
  --layout-features="$FEATURES" --no-hinting --desubroutinize
...
echo
for f in "$OUT/fraunces-latin.woff2" "$OUT/fraunces-italic-latin.woff2" "$OUT/inter-latin.woff2"; do
