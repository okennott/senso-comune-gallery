...
# Chrome writes the capture at 2x, which is 488 dpi where the report places it.
# soft_plate.py resamples it to 300 dpi and keeps it LOSSLESS: a screenshot is
# mostly type and UI edges, and a JPEG artefact in a capture of a now-textured
# page cannot be told apart from the texture itself.
shoot() {  # name route width height scale
  ...
  python3 "$HERE/soft_plate.py" page "$HERE/fig/cap-$1.png" "$HERE/fig/cap-$1.png"
  printf '  fig/cap-%s.png  %s  %sx%s @%sx → 300 dpi, lossless\n' "$1" "$2" "$3" "$4" "$5"
}
