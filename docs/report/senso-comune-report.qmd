...
| Contrast | **AA, on the wall.** The plates are off and the wall is dark, so the ink ladder was re-solved for it the same evening: `--body` measures **4.92:1** on `--field` and `--muted` 4.62:1, against a floor of 4.5 (@sec-ink). The contrast check now solves the wall as an ordinary ground, so a regression blocks the build |
...
2026 there is also a shape**, traced from the drawing for the surfaces that
cannot hold a photograph; what that costs is @sec-mark-vector, and the
masthead is not one of them.
...
The **masthead is deliberately not one of them**. The artwork's own cream
paper is the plate there, which is the site's central visual idea at wordmark
size, and at 34 px on a phone the drawing still holds. Swapping it is one line
in `templates.js` and it is Priscilla's decision, not the build's — and the
first place to look before making it is whether the vector source exists.
...
page. All from `brand/mark-lockup.png`, by one crop. Beside them, from that
same crop by way of the reduction in @sec-mark-vector: `icon.svg`, which the
tab is offered first, and `mark-sc.svg`, which carries no colour and is there
for anything that has to supply its own.
...
| `check-review` | **92 assertions over 58 findings** — the design review, the mark, softness, the shop bar and views, the structure, the flags and the gate — written against the built pages, so a fix that stops being applied fails the build |
