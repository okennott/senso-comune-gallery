...
It is now framed in both the places About appears, and the two places take it
differently because the pages are different shapes.

**On the homepage** the frame sits in the heading column, under *About*, so it
stands beside the prose at every width that column is beside it and above the
prose on a phone. It needs no breakpoint of its own: the section head is
already a two-column grid at 760 px and the portrait is simply the second
thing in the first column.

**On the About page** it is a frontispiece, over the title. It could not be
anything else. That page's mount is 660 px around a 34 rem measure — finding
D-01 brought it in to the column, because an 860 px mount left a third of it
empty — and 660 less the 544 px the measure takes leaves 116 px, which is a
margin, not a plate. A book with a portrait in it puts it on the leaf facing
the title page, and that is the arrangement the page already has.
...
in `--muted` at 4.62:1 on the wall.
...
Four assertions, M-01: that both places carry the frame in both languages, the
frontispiece before the title; that it is matted like a painting, carries its
own width and height so it costs no layout shift, and is in no way a work; that
an empty frame is marked as a placeholder and is gated out of a release; and
that the description rule is unwaivable, conditional and owns its own field.
...
![About, set as a book page: the display face at 19.2 px, a lede, a three-line drop cap in sanguine, and the closing quotation in Fraunces' real italic. The mount comes in to 660 px around a 34 rem column, and with its plate transparent it is the shadow and the rim that draw it. Over the title, the artist's portrait as a frontispiece, matted as a painting is and holding the pipeline's placeholder card until the photograph exists (@sec-portrait). The Chinese page is the same page in a system serif, without the drop cap or the italic (@sec-about-type).](fig/cap-about.png){#fig-about width=100%}
...
| `check-review` | **88 assertions over 57 findings** — the design review, the mark, softness, the shop bar and views, the structure, the flags and the gate — written against the built pages, so a fix that stops being applied fails the build |
