...
: The site on 20 September 2026 {#tbl-status}
...
| Contrast | **AA, on both grounds.** The plates are off and the wall is dark, so the ink ladder was re-solved for it: `--body` measures **4.92:1** on `--field` and `--muted` 4.62:1, against a floor of 4.5 (@sec-ink). About is now a painted panel in a darker green still, which admits exactly one ink — 4.55:1, where pure black would reach 4.65 (@sec-panel). The contrast check solves both as ordinary grounds, so a regression on either blocks the build |
...
legible reading surface on the site. The sheet itself keeps its shadow and its
rim light, which are the only things left saying that the content column is
mounted on the field rather than cut out of it.
...
It is now a **band**, full width, immediately under the masthead: the wall
itself, held open by its padding, with the four brand colours ruled across its
foot and an engraved line closing it. The hero below carries nothing but the
painting.
...
**The size on a phone.** The motto may never be re-wrapped by the browser —
the attribution is aligned to the real right edge of its longest line, and a
browser cannot shrink a box to the longest line of wrapped text. So its size
on a narrow screen is decided by that line: *"with love is well done."* is
400 px at 38.4 px, 296 at 27.6, and 255 at 23.8, against columns of 480, 350
and 280. Two steps, at 519 px and 359 px, and `check-render` measures the
rendered line count against the `<br>`s at five widths and fails the build on
a re-wrap.
...
The four rules at its foot are the sheet's own brand colours in the order they
can be seen on sage — the accent, her deep green, the bar's brown, the cream —
and they are decoration that says so: inside an `aria-hidden` wrapper, no
link, no text, nothing carried by any of them but the palette.
...
![The homepage as it now stands, with the plates off (@sec-plates): one sage field from the bar to the footer, the content column visible only as a shadow and a rim light, and cream kept for the mat around the work. Dark brown bar, the SC monogram at the head of the wordmark — the traced vector, in the wordmark's own cream (@sec-mark-vector) — and search, account and cart at the right. Under the bar, the band: van Gogh's line stands twice, his Dutch from letter 143 to Theo — *wat met liefde gedaan wordt dat wordt goed gedaan* — set small and italic above the reader's own language, with his full name beneath, aligned to the quote's longest line, and the four brand colours ruled across its foot (@sec-band). The featured work follows below it, and each section boundary is stated by an engraved line (@sec-soft-line).](fig/cap-home.png){#fig-home width=100%}
...
| `check-review` | **99 assertions over 62 findings** — the design review, the mark, softness, the shop bar and views, the structure, the flags and the gate — written against the built pages, so a fix that stops being applied fails the build |
