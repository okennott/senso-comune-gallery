...
| Contrast | **Not AA as it stands.** The reading plates were switched off on the evening of 20 September: text is read on the sage field itself, where `--body` measures 2.35:1 against a floor of 4.5. A decision, recorded, measured and reversible in four tokens (@sec-plates) |
...
|:----------------|-----------:|-----------:|----------:|------:|
| `--ink` headings | 11.56 | 4.70 | 3.09 | 4.5 |
| `--body` running text | 5.78 | **2.35** | **1.55** | 4.5 |
| `--muted` captions | 4.94 | **2.01** | **1.32** | 4.5 |
| `--sanguine` links | 5.70 | **2.32** | **1.52** | 4.5 |
| `--muted-ui` strokes | 3.27 | **1.33** | **1.14** | 3.0 |
...
`#D7DAC6`, where `--body` measures **4.76:1** at the top of the fall and 4.38 at
...
field at 2.35:1, 2.01:1 and 2.32:1 (@tbl-plates-cost). That is below 1.4.3's
...
| `check-review` | **82 assertions over 54 findings** — the design review, the mark, softness, the shop bar and views, the structure, the flags and the gate — written against the built pages, so a fix that stops being applied fails the build |
