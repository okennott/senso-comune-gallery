...
adopted on 18 September 2026 from her prototype sheet, which arrives with a
construction, a palette and a set of variations already worked out.
...
### What the sheet specifies, and what is adopted {#sec-mark-sheet}

```{=latex}
\Needspace*{14\baselineskip}
```

: The prototype sheet against the built mark {#tbl-mark}

| On the sheet | In the build |
|:--------------------------------|:-----------------------------------------------|
| Interlocking S and C | Adopted. The S sits upper left, the C lower right, overlapping by 45 % of the S's width |
| Letter proportion S : C = 1 : 1.618 | Adopted as 1 : $\sqrt{\varphi}$ = 1 : 1.272 — see below |
| Fibonacci / golden-rectangle grid | Adopted as the layout: the pair is sized to fill a square field with equal margins on four sides |
| Diagonal hatching, upper left → lower right | Adopted at 45°, which is the angle and the direction the Leonardo hatch already runs at across the Tribute ground. Off below about 96 px |
| Sage green `#969B7D` | Already `--field` |
| Deep green `#737C61` | Already `--field-deep`; it is what the hatching is drawn in |
| Warm ivory `#E9E2D3` | **Not added.** Within one step of `--wash-blue` `#E9E3D6`; a fifth cream is a fifth cream |
| Charcoal black `#24241F` | **Not adopted.** The site's ink and bar are `#3A2B22`, a warm brown the whole palette is solved against; a cool near-black beside sage reads as a second, colder identity. The sheet's *black* variation is drawn in `--bar` |
| Minimum size 15 mm, print | Adopted as the reason the hatching comes off, and extended: the mark is also cut at 32 px and 16 px, where 15 mm has no meaning |

### Two decisions taken here {#sec-mark-decisions}

**The letters are Fraunces.** There is no vector source for the drawing on the
sheet, and a trace from a screenshot would be a worse object than a
construction. Fraunces is the face the wordmark is already set in; at the
display end of its optical-size axis (`opsz 40`, `wght 700`) it draws the heavy
stems against fine hairlines that the prototype is built from. So the lockup —
the mark beside *Senso Comune* — is one typeface rather than two. The outlines
are cut from the font by `scripts/build-mark-paths.py` and committed as
`scripts/mark-paths.js`, so building the site needs Node and nothing else.

**The proportion is $\sqrt{\varphi}$, not $\varphi$.** This is the one number
on the sheet not taken literally, and the reason is visible rather than
theoretical. Drawn at 1 : 1.618 in a square field, Fraunces' S falls to about
the C's x-height and stops reading as a capital — the mark reads *sC* — and
the C's lower arm cannot reach far enough left to close the hole the small S
leaves at the bottom of the square. At 1 : 1.272 the S is still plainly the
smaller letter, which is what the sheet's own four variations show, and the
pair fills the square. $\sqrt{\varphi}$ is the same construction one step in:
the golden section of the golden section.

### Four renderings, one geometry {#sec-mark-two}

![The monogram as the site ships it. Every tile is the SVG `scripts/mark.mjs` produces, rasterised here rather than redrawn, so the report cannot argue for a mark the site no longer has.](fig/mark-plate.pdf){#fig-mark width=100% fig-pos="H"}

The C's cap height is 68 % of the field and the S's is 54 %; the two are pushed
together by 45 % of the S's width and the pair is then sized to leave the same
margin on all four sides. Where the S crosses the C there is an **inlay** — a
gap the width of twice `--inlay`, which is what makes the C pass *behind* the S
rather than merge with it. On the icon that gap is stamped in the field colour;
on the masthead, where there is no field to stamp with, it is cut with a mask,
so the bar — or the painting passing under the bar once it goes to glass —
shows through it.

That distinction matters more than it sounds. The masthead paints **no field**
at all: an opaque square behind the letters would have been invisible while the
bar was solid and would have appeared as a slab the moment the bar became
glass (@sec-soft).

### What it deliberately does not do {#sec-mark-not}

- **It spends no accent.** Sanguine is 2.03:1 on the bar and would disappear at
  tab size. The letters are `--paper` on `--bar` at **11.45:1**, the most
  legible pair on the site, and the accent stays reserved for calls to action.
- **It depends on no font at render time.** An SVG favicon is rendered
  *without* the page's webfonts, so a mark set as `<text>` falls back to
  whatever serif the operating system has — a different logo on every machine.
  These are outlines, cut at build time. The check forbids live text and
  requires outlines; it does not forbid letterforms any more.
- **It has no per-locale variant**, by construction, for the reason in
  @sec-mark-constraint.
- **It does not carry its texture down to sizes the texture cannot survive.**
  The bottom row of @fig-mark is the argument: the hatching is gone by 32 px
  and would be mud at 16 px.

WCAG contrast is defined on relative luminance, so converting the mark to
greyscale leaves its ratio exactly where it is and it survives a monochrome
print unchanged. Two cases have **not** been tested: single-ink reproduction of
the hatched rendering, where hatch and letter cannot be separated by lightness
at all, and embroidery below the sheet's own 15 mm. Neither is needed before
launch.

### Where it is used {#sec-mark-where}

The icon set — `icon.svg` at 3.6 KB, the 32 px `favicon.ico`, the 180 px
home-screen icon and the manifest — all from one source; the masthead lockup,
where the mark and the name are a single link and a single tab stop; and this
document, in the footer of every page and on the title page, included as the
vector `mark.mjs` produces rather than redrawn in TikZ, which is what the
footer carried while the mark was three rectangles.

Eight assertions in `scripts/check-review.mjs` (F-01 to F-04) hold it together:
that the icon is the geometry `mark.mjs` declares, that the masthead is the
same monogram with its field unpainted and its inlay masked, that the letters
are Fraunces outlines from a stated instance and not live text, that the mark
contributes no accessible name of its own, that it is byte-identical in both
locales, that it spends no accent and holds its proportions at tab size, that
the hatching is off in both the icon and the masthead while remaining
available, and that the ratio is derived from $\varphi$ rather than typed in.
...
scripts/         mark.mjs · mark-paths.js     the monogram, shared with
                                              this report; its letters are
                                              cut by build-mark-paths.py
...
| `check-review` | **70 assertions over 46 findings** — the design review, the mark, softness, the shop bar and views, the structure, the flags and the gate — written against the built pages, so a fix that stops being applied fails the build |
...
| A mark for two languages | An `SC` monogram — a device beside the name rather than a translation of it — cut from Fraunces outlines and generated from one geometry for the tab, the home screen, the masthead and this document | `scripts/mark.mjs` |
...
**The mark.** @fig-mark and the mark in this document's footer are the SVG
that `mark.mjs` produces — asked of it at build time and included as a vector,
not redrawn here. The letters are cut out of the shipped Fraunces subset by a
generator script at a stated instance, so the document, the browser tab and
the masthead cannot be three different drawings.
