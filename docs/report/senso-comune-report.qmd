...
: The site on 17 September 2026 {#tbl-status}
...
The first icon was `SC`, cut from real Fraunces outlines. It read at 16 px, and
it served only one of the two audiences: the problem is *make something that
reads at 16 px in two languages*.
...
Seven assertions in `scripts/check-review.mjs` (F-01 to F-04) hold it together: that the icon
...
- Six assertions in `check-review.mjs` (G-01 to G-06) cover the rest: no
  ...
  grounds actually either side of it.
...
- **Series are data.** A series is one entry in `site.json`; its page, its tab and
  ...
  `/works/`.
- **The homepage is a front door, not the catalogue:** van Gogh's line beside
  one *Featured work*, the four *Latest works* with a link to all of them and
  their number, a card for each series, About, and How to Buy.
...
![The homepage. Cream sheet mounted on the sage field, dark brown bar, the mark at the head of the wordmark, and search, account and cart at the right. Van Gogh's line, in his own words from letter 143 to Theo — *that which is done with love is well done* — is attributed beneath it, aligned to its longest line, beside the featured work; on a phone the line leads and the work follows.](fig/cap-home.png){#fig-home width=100%}
...
declined, and the wordmark still translates — which is what later decided the
logo (@sec-mark). And half of **B-03** was wrong: the scale diagram's painting
and sofa were said to be off-axis, and are centred. Its contrast half was real,
and is the measurement above.
...
with `sharp` the only dependency — which is the only architecture consistent
with a design whose argument is restraint.
...
scripts/         mark.mjs                     the logo, shared with this report
...
`npm run check` runs the first four anywhere, in seconds. The rendering check
needs Chrome; GitHub runs it on every preview, and the release runs all of them.
...
| `check-review` | **66 assertions over 46 findings** — the design review, the mark, softness, the shop bar and views, the structure, the flags and the gate — written against the built pages, so a fix that stops being applied fails the build |
| `test-readiness` | Any of **43 tests** showing the gate opens only when it should (@sec-readiness-proof) |
| `check-render` | In Chrome, at 1440, 900, 390, 360 and 320 px across 12 pages: a painting out of its own proportions, rounded or transformed; a masthead that overflows, takes more than two rows or has a control under 44 px; van Gogh's line re-wrapped, its attribution off its edge, or the featured work ahead of it on a phone |
...
| Account | A server, a login, password recovery and a privacy regime, for an order history the payment provider already keeps | Reasonable only if the site moves to a hosted commerce platform that supplies accounts; not scheduled | — |
...
Six places where the evidence is thinner than the decision resting on it. Every
one needs a person outside this project — a customs broker, a tax adviser, a
carrier, a payment provider — and none can be closed by writing more code.
...
| A mark for two languages | A mark that carries no letterforms, so one file serves both locales, generated from a single geometry for the tab, the home screen, the masthead and this document | `scripts/mark.mjs` |

## Content still outstanding

The 54 details in @sec-readiness-today. The blocking ones that matter most are
