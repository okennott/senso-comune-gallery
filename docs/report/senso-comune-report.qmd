...
| Contrast | **Not AA as it stands.** The reading plates were switched off on the evening of 20 September: text is read on the sage field itself, where `--body` measures 1.98:1 against a floor of 4.5 — 2.35:1 until the wall was re-solved darker on the same day (@sec-wall). A decision, recorded, measured and reversible in four tokens (@sec-plates) |
...
**The plates are off.** Later the same evening the reading surfaces were made
transparent, so the field is the only background colour on the site. The
lightness ladder that made those surfaces readable is still in the token file
and still solved; it is simply not painted. What that costs is measured rather
than estimated, and the contrast check prints the bill on every run without
blocking the build (@sec-plates).
...
: What every reading pair measures with the plates off. The first column is the ground the palette was built for and still solves against. {#tbl-plates-cost}

| Token | On `--wash-pale` | On `--field` | On `--field-deep` | Floor |
|:----------------|-----------:|-----------:|----------:|------:|
| `--ink` headings | 11.56 | 3.96 | 2.65 | 4.5 |
| `--body` running text | 5.78 | **1.98** | **1.33** | 4.5 |
| `--muted` captions | 4.94 | **1.69** | **1.13** | 4.5 |
| `--sanguine` links | 5.70 | **1.95** | **1.31** | 4.5 |
| `--muted-ui` strokes | 3.27 | **1.12** | **1.33** | 3.0 |

That is quietness bought with contrast, which is the one thing this report
opens by forbidding: the draft failed its review because every call to action
was `#A19E97` at 2.67:1, and `--body` on the field is worse than that at both
ends of the page. It is recorded here in those terms because a decision taken
with the number in front of you is a different act from the one the draft made.

**What the check does about it.** `check-contrast.mjs` reads the plate tokens.
When any of them is transparent it prints @tbl-plates-cost as
*"THE PLATES ARE OFF — what the page actually renders"*, marks every pair that
fails, and states that it cannot certify the page as AA while they are off. It
does **not** count them as failures and does not block the build: painting a
plate is a decision, the token file is where that decision is taken, and a
check that overrules a decision is a check that gets edited. What it will no
longer do is finish by saying *all pairs pass against their worst-case ground*
— with the plates off it says they pass against plates that are not painted.
...
**It costs contrast, and the bill is above.** A darker wall under the same ink
is a smaller ratio: `--ink` goes 4.70 → **3.96**, `--body` 2.35 → **1.98**, and
@tbl-plates-cost is stated in the new numbers throughout. With the plates
painted none of this would signify — the reading grounds are unchanged, and
every pair on them still measures what @tbl-sage-ground says. It signifies
because the plates are off, which is the other half of the same decision.
...
|:-----------------|:-------------------------------|-----------:|-----------:|
| `--wash-pale` | `--muted-ui` at 3.27:1 | ±0.283 | ±0.250 |
| `--wash-mid` | `--muted-ui` at 3.15:1 | ±0.126 | ±0.250 |
| `--wash-deep` | `--muted-ui` at 3.06:1 | ±0.052 | ±0.250 |
...
```{=latex}
\begin{finding}{1.4.3 does not hold as the site now stands}
```

The row above is the work as it was built, and every token in it is still
solved. What changed on the evening of 20 September is the **ground**: with the
reading plates transparent, body text, captions and links are read on the sage
field at 1.98:1, 1.69:1 and 1.95:1 (@tbl-plates-cost). That is below 1.4.3's
4.5:1 and, for the muted pair, below 1.4.11's 3:1 as well.

It is a deliberate visual decision, not a regression that slipped through — the
contrast check prints it on every run — but it is the one item in this section
a release cannot carry unexamined. Restoring the plates restores every figure in
the table above exactly; see @sec-plates.
...
| `check-review` | **83 assertions over 55 findings** — the design review, the mark, softness, the shop bar and views, the structure, the flags and the gate — written against the built pages, so a fix that stops being applied fails the build |
