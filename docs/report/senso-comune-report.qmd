...
That sheet was cream for the first three months of the project. It is sage now,
and the cream has gone back to being the mat: @sec-sage-ground.
...
report is printed on `--wash-pale`, flat, which is exactly what the site's
reading surfaces are. It also removes a 300 dpi image from underneath every
page, which is one less thing between the press and the type.
...
- Four more (L-01 to L-04) cover the decisions of 20 September 2026: the fifteen
  ...
  the italic a real file with synthesis off; and About set as a book page in
  both languages, including the two refusals on the Chinese one.
...
![About, set as a book page: the display face at 19.2 px, a lede, a three-line drop cap in sanguine, and the closing quotation in Fraunces' real italic. The mount comes in to 660 px around a 34 rem column. The Chinese page is the same page in a system serif, without the drop cap or the italic (@sec-about-type).](fig/cap-about.png){#fig-about width=100%}
...
| `check-review` | **78 assertions over 52 findings** — the design review, the mark, softness, the shop bar and views, the structure, the flags and the gate — written against the built pages, so a fix that stops being applied fails the build |
