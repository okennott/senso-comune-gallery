...
author: "Prepared for the artist"
...
**Adopt the artist's sage palette**, with text moved onto cream.
...
The artist supplied a nine-page draft and, later, a live mockup with a revised
...
The artist's mockup moves the site from white to a sage-green world: a dark
...
![Why the sage carries no text. The three red bars are the artist's mockup as drawn; the draft's original failure is included for scale.](fig/sage-limit.pdf){#fig-sage width=100%}
...
: Type scale, and how close the artist's draft already was {#tbl-scale}
...
- **The accent is sanguine**\index{sanguine}. Red chalk, the medium Leonardo
  drew in, measures 6.33:1 on white; the oxblood the artist chose by eye
...
```{=latex}
\begin{blocker}{What she must not do}
```
...
| 6 | CJK webfont strategy | Deferred to a system stack | 5–10 MB vs 80 KB; decide before translating |
| 7 | Leonardo's hatching angle | Original measurement, not a citation | Present as measured |
| 8 | Mona Lisa glaze layer count | Not measurable by XRF; the authors say so | Never write "thirty layers" |
