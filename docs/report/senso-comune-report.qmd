...
scripts/        mark.mjs          the logo geometry, shared with the report
                ...
                check-contrast · check-links · check-review
...
Three, and `npm run check` runs all three.
...
`check-review.mjs` is newer and differently shaped: **41 assertions over the
24 findings** of @sec-review, written against the built pages rather than the
source, so a fix that silently stops being applied fails the build instead of
being noticed in a screenshot months later.
